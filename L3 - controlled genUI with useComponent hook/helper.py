...
def install_frontend(frontend_dir="frontend"):
    ...
    if proc.returncode != 0:
        raise subprocess.CalledProcessError(proc.returncode, [npm_cmd, "install"])
...
def start_frontend(port, frontend_dir="frontend"):
    ...
    proc = subprocess.Popen(
        [npm_cmd, "exec", "--", "concurrently", "node --import tsx/esm watch-server.ts", f"vite --port {port}"],
        cwd=frontend_dir,
        env=env,
        stdout=log_file,
